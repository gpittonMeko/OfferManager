...
class EmailCampaignSend(TimestampMixin, db.Model):
    ...
    contact_id = db.Column(db.Integer, db.ForeignKey("contacts.id"), nullable=True)
    lead_id = db.Column(db.Integer, db.ForeignKey("leads.id"), nullable=True)
    status = db.Column(db.String(20), default="pending")  # pending, sent, delivered, opened, clicked, bounced, failed
    mailgun_message_id = db.Column(db.String(200))
    sent_at = db.Column(db.DateTime)
    ...
    if request.method == 'GET':
        ...
        return jsonify({
            'accounts': [{
                ...
                'opportunities': [{'id': o.id, 'name': o.name, 'stage': o.stage, 'amount': o.amount} for o in a.opportunities]
...
def pipeline_analytics():
    ...
    # Raggruppa per periodo e categoria
    data = {}
    ...
    for opp in opps:
        ...
        # Determina categoria dai prodotti
        category = 'Altro'
        if opp.line_items:
        ...
        if is_closed:
            data[period_key][category]['closed'] += opp.amount
        else:
            data[period_key][category]['open'] += opp.amount
    
    return jsonify({'data': list(data.values())})
...
def send_email_campaign(campaign_id):
    ...
    try:
        ...
        # Prepara destinatari dalla lista
        ...
        if not subscriptions:
            return jsonify({'error': 'Nessun destinatario trovato nella lista'}), 400
        
        # Prepara dati per Mailgun con nomi normalizzati
        recipients = []
